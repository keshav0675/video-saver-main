...
def format_error_message(error: Exception, platform: str = None) -> str:
    ...
    # Check for YouTube bot challenge / cookies required
    if 'bot' in error_str or 'sign in' in error_str or 'cookies' in error_str:
        return (
            "❌ <b>YouTube Anti-Bot Block:</b>\n\n"
            "YouTube is requiring authentication for this server. "
            "Please configure <code>cookies.txt</code> or try another link."
        )
    ...
    if 'no video formats found' in error_str or 'keyerror' in error_str or 'extractor error' in error_str:
        return "❌ <b>Extraction Failed:</b> Unable to parse this video. The website structure may have changed or content is restricted."
